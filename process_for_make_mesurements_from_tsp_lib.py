...
NAME_OF_DATASET_TSP_LIB_DIR = "TSPLIB"
...
PATTERN_FOR_TSP_LIB_SRC_FILE = "%s.tsp"
PATTERN_FOR_TSP_LIB_OPT_TOUR_FILE = "%s.opt.tour"
DISTANCE = 1000
...
NAME_OF_DIR_FOR_MEASUREMENTS = "measurements/verification_machinery/PC1_HELD_KARP"
...
def measure_sequence():
    total = len(NAME_OF_DIR_FOR_MEASUREMENTS) * len(NAMES_OF_ALGORITHMS) * len(
        CONFIGURATION_LIST_OF_DICT) * len(TYPE_OF_MEASUREMENT)
    current = 0
    start = time.time()
    for alg in NAMES_OF_ALGORITHMS:
        for name_of_tsp_issue in NAMES_OF_TSP_ISSUES:
            for config_dict in CONFIGURATION_LIST_OF_DICT:
                for type_of_measure in TYPE_OF_MEASUREMENT:
                    path_to_src_tsp_lib = PathBuilder() \
                        .add_dir(NAME_OF_DATASET_DIR) \
                        .add_dir(NAME_OF_DATASET_TSP_LIB_DIR) \
                        .add_file_with_extension(PATTERN_FOR_TSP_LIB_SRC_FILE % name_of_tsp_issue) \
                        .build()
                    path_to_opt_tour_tsp_lib = PathBuilder() \
                        .add_dir(NAME_OF_DATASET_DIR) \
                        .add_dir(NAME_OF_DATASET_TSP_LIB_DIR) \
                        .add_file_with_extension(PATTERN_FOR_TSP_LIB_OPT_TOUR_FILE % name_of_tsp_issue) \
                        .build()
                    path_to_output_csv = PathBuilder() \
                        .add_dir(NAME_OF_DIR_FOR_MEASUREMENTS) \
                        .create_directory_if_not_exists() \
                        .add_file("%s_%s" % (type_of_measure, name_of_tsp_issue), CSV) \
                        .build()
                    csv_manager = CsvManager(path_to_csv=path_to_output_csv)
                    tsp_input_data = TspInputData(path_to_src_tsp_lib)
                    ...
                    collector.add_data(CsvColumnNames.NUMBER_OF_CITIES, tsp_input_data.number_of_cities)
                    collector.add_data(CsvColumnNames.NAME_OF_TSP_ISSUE, name_of_tsp_issue)
                    ...
                    collector.add_data(CsvColumnNames.NAME_OF_SRC_FILE, name_of_tsp_issue)
                        ...
                        if not tsp_input_data.is_valid_way_for_any_type(
                            ...
                            collector.add_data(HAMILTONIAN_CYCLE_COST,
                                               tsp_input_data.cal_total_distance(
                                                   collector.get_dictionary_with_data()[BEST_WAY]))
                            tsp_optimal_verifier = TspOptimalVerifier(src_tsp_file_name=path_to_opt_tour_tsp_lib,
                                                                      tsp_path_to_verify=
                    ...
                    title = "ALG: %s | N: %d | name: %s | measure: %s | suffix: %s | time_duration: %s" % (
                        alg, tsp_input_data.number_of_cities, name_of_tsp_issue, measure, config_dict[SUFFIX], print_diff_time(diff))
