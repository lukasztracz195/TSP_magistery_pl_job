...
def iter():
    ...
    for i in range(n):
        if cnt < 5:
            ...
            plt.text(coor[i][0], coor[i][1] - 3000, str(i), fontsize=8, ha="center", va="center", color='black',
                     stretch='condensed')
        else:
            map.plot(coor[i][0], coor[i][1], marker='o', color='black', markersize=3)
            plt.text(coor[i][0], coor[i][1], cities[i], fontsize=8, ha=pos[i][0], va=pos[i][1], color='black',
                     stretch='condensed')
